  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 sm:top-4 left-3 sm:left-4 flex flex-col gap-1.5 sm:gap-2 z-10">
        {item.isNew && (
          <motion.div
            ...
            className="px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-gray-900 flex items-center gap-1 shadow-sm"
          >
        ...
        {item.isBestseller && (
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-gray-700 flex items-center gap-1 shadow-sm"
          >
            <TrendingUp className="w-2.5 sm:w-3 h-2.5 sm:h-3 text-white" />
        ...
        {item.isLimited && (
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-gray-500 flex items-center gap-1 shadow-sm"
          >
            <Zap className="w-2.5 sm:w-3 h-2.5 sm:h-3 text-white" />
      ...
      <motion.button
        ...
        className="absolute top-3 sm:top-4 right-3 sm:right-4 z-10 w-8 sm:w-10 h-8 sm:h-10 rounded-full bg-white border border-gray-200 flex items-center justify-center hover:border-gray-300 transition-colors"
      >
        <Heart className={`w-4 sm:w-5 h-4 sm:h-5 ${isLiked ? 'fill-red-500 text-red-500' : 'text-gray-700'}`} />
      </motion.button>
      
      {/* Discount Badge */}
      {discount > 0 && (
        <div className="absolute top-3 sm:top-4 right-12 sm:right-16 z-10 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-gray-900 shadow-sm">
          <span className="text-xs font-bold text-white">-{discount}%</span>
        </div>
      )}
      
      {/* Image Section */}
      <div className="relative h-48 sm:h-64 bg-gray-50 flex items-center justify-center overflow-hidden">
        <motion.div
          whileHover={{ scale: 1.05 }}
          transition={{ duration: 0.3 }}
          className="w-12 sm:w-16 h-12 sm:h-16 rounded-2xl bg-gray-900 flex items-center justify-center"
        >
          {item.icon ? <item.icon className="w-6 sm:w-8 h-6 sm:h-8 text-white" /> : null}
        </motion.div>
      ...
      <div className="p-4 sm:p-6 relative flex flex-col flex-1">
        ...
        <div className="flex items-center gap-2 mb-2 sm:mb-3">
          <Tag className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-gray-600" />
        ...
        {/* Title */}
        <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-2 line-clamp-2 min-h-[3rem] sm:min-h-[3.5rem] group-hover:text-gray-700 transition-colors">
          {item.name}
        </h3>
        
        {/* Description */}
        <p className="text-xs sm:text-sm text-gray-600 mb-3 sm:mb-4 line-clamp-2 min-h-[2.5rem]">
          {item.description}
        </p>
        ...
        <div className="mb-3 sm:mb-4">
          {item.sizes && item.sizes.length > 0 && (
            <>
              <label className="text-xs text-gray-600 mb-1.5 sm:mb-2 block">{t('merch.size')}:</label>
              <div className="flex flex-wrap gap-1.5 sm:gap-2 min-h-[2rem]">
                {item.sizes.map(size => (
                  <button
                    key={size}
        ...
        <div className="mb-3 sm:mb-4">
          {item.colors && item.colors.length > 0 && (
            <>
              <label className="text-xs text-gray-600 mb-1.5 sm:mb-2 block">{t('merch.color')}:</label>
              <div className="flex flex-wrap gap-1.5 sm:gap-2 min-h-[2rem]">
        ...
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <div className="flex flex-col">
            <div className="flex items-baseline gap-1.5 sm:gap-2">
              <span className="text-xl sm:text-2xl font-bold text-gray-900">{item.price.toLocaleString()} {t('common.currency')}</span>
              {item.originalPrice && (
                <span className="text-xs sm:text-sm text-gray-500 line-through">{item.originalPrice.toLocaleString()} {t('common.currency')}</span>
              )}
            </div>
